<template>
  <div class="main-content">
    ...
    <el-dialog
      ...
      width="800">
        <div id="zhuceshijianChart" style="width:100%;height:600px;"></div>
...
export default {
  ...
  methods: {
    ...
    chartDialog() {
      ...
      this.$nextTick(()=>{
        var zhuceshijianChart = this.$echarts.init(document.getElementById("zhuceshijianChart"),'macarons');
        this.$http({
            url: "group/xuesheng/zhuceshijian",
            method: "get",
        }).then(({ data }) => {
            if (data && data.code === 0) {
            let res = data.data;
            ...
            for(let i=0;i<res.length;i++){
                xAxis.push(res[i].zhuceshijian);
                yAxis.push(res[i].total);
                pArray.push({
                value: res[i].total,
                name: res[i].zhuceshijian
                })
                var option = {};
                option = {
                    title: {
                ...
                // 使用刚指定的配置项和数据显示图表。
                zhuceshijianChart.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    zhuceshijianChart.resize();
                };
            }
            }
        });
